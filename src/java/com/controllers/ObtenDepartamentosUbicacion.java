...
public class ObtenDepartamentosUbicacion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String queryDepsUbi = "SELECT departments.department_id, " +
                ...
                "regions.region_id, " +
                "regions.region_name " +
                "FROM departments, locations, employees, countries, regions " +
                "WHERE locations.location_id = departments.location_id " +
                "AND departments.manager_id = employees.employee_id " +
                "AND locations.country_id = countries.country_id " +
                "AND countries.region_id = regions.region_id " +
                "AND departments.location_id = " + idUbicacion;
        
        try {
            conn = DBUtil.getProxoolConexion();
            Statement sentencia = conn.createStatement();
            ResultSet resultado = sentencia.executeQuery(queryDepsUbi);
            ...
            if (departamentosUbicacion.isEmpty()) {
                response.sendRedirect("NoDepartamentosUbicacion.jsp");
            }
            else{
                session.setAttribute("depaLocation", departamentosUbicacion);
                response.sendRedirect("MuestraDepartamentosUbicacion.jsp");
            }
